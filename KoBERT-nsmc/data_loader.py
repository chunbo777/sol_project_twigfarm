...
class NsmcProcessor(object):
    ...
    def _create_examples(self, lines, set_type):
        ...
        examples = []
        #for aihub
        df =  pd.read_csv(lines, names=["index", "line", "label"])
        for i in range(len(df["line"][1:])):
            text_a = df["line"][i+1]
            label = int(df["label"][i+1])
            guid = "%s-%s" % (set_type, i)


        # for line, label in zip(df["line"],df["label"]):
        #     text_a = line
        #     label = label
            if i % 1000 == 0:
                logger.info(text_a)
    ...
    def get_examples(self, mode):
        ...
        return self._create_examples(os.path.join(self.args.data_dir, file_to_read), mode)
